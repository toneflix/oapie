...
import { generateTypeScriptModule } from './TypeScriptGenerator'
...
export const serializeOutput = async (
    ...
    if (outputFormat === 'ts') {
        return prettier.format(generateTypeScriptModule(payload as never, rootTypeName), {
            parser: 'typescript',
            semi: false,
            singleQuote: true,
        })
...
export const buildOutputFilePath = (
...
export const getRootTypeNameForShape = (shape: string): string => {
